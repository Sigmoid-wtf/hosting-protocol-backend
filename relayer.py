...
OLAS_MANAGER_CONTRACT_ADDRESS = '0x235107eb76d0eC9280204eF328ef9f6aA36a07E5'
...
class OlasManagerBackend:
    def __init__(self, owner_private, olas_token, contract, w3_gno):
        self.owner = w3_gno.eth.account.from_key(owner_private)
    ...
    def run_node(self, node: Node):
        ...
        node.service_id = int(self.read_service_id(node))
...
def main():
    ...
    manager = OlasManagerBackend(
        ...
        w3_gno.eth.contract(address=OLAS_MANAGER_CONTRACT_ADDRESS, abi=OLAS_MANAGER_CONTRACT_ABI),
        w3_gno,
    )
    token_contract = w3_gno.eth.contract(address=OLAS_CONTRACT_ADDRESS, abi=OLAS_CONTRACT_ABI)

    # stake
    stake_amount = manager.w3_gno.to_wei(20, "ether")
    ...
    if token_contract.functions.balanceOf(OLAS_MANAGER_CONTRACT_ADDRESS).call() - contract_olas_locked_amount >= stake_amount:
        tx = manager.contract.functions.lockStaking(stake_amount).build_transaction({
            ...
            "nonce": w3_gno.eth.get_transaction_count(manager.owner.address)
        })
        print(tx)
        signed_tx = manager.owner.sign_transaction(tx)
        tx_hash = w3_gno.eth.send_raw_transaction(signed_tx.rawTransaction)
        w3_gno.eth.wait_for_transaction_receipt(tx_hash)
        ...
        node.contract_id = manager.contract.functions.nextNodeId().call() - 1
